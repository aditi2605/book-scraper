...
public class BooksController : ControllerBase
{
    ...
    public BooksController(
        AppDbContext db, IHubContext<ScrapingHub> hubContext,
        HapScraperService hapScraper, PlaywrightScraperService playwrightScraper,
        ILogger<BooksController> logger)
    {
        _db = db; _hubContext = hubContext;
        _hapScraper = hapScraper; _playwrightScraper = playwrightScraper;
        _logger = logger;
    }

    [HttpPost("scrape")]
    public async Task<ActionResult> StartScraping(
        [FromQuery] string engine = "hap", [FromQuery] int? maxPages = null)
    {
        ...
        return Ok(new { message = $"Scraped {books.Count} books using {scraper.EngineName}", count = books.Count, engine = scraper.EngineName });
    }

    [HttpGet]
    public async Task<ActionResult<List<Book>>> GetAllBooks()
    {
        return Ok(await _db.Books.OrderBy(b => b.Title).ToListAsync());
    }

    [HttpGet("search")]
    public async Task<ActionResult<List<Book>>> SearchBooks(
        [FromQuery] string? q, [FromQuery] double? minPrice, [FromQuery] double? maxPrice,
        [FromQuery] int? minRating, [FromQuery] bool? inStock,
    ...
    public async Task<ActionResult<BookAnalytics>> GetAnalytics()
    {
        ...
        if (!books.Any()) return Ok(new BookAnalytics());

        var ratingDist = books.GroupBy(b => b.Rating)
            .Select(g => new RatingGroup
            {
                Rating = g.Key, Count = g.Count(),
    ...
    {
        var books = await _db.Books.ToListAsync();
        if (!books.Any()) return Ok(new List<BookDeal>());

        var maxPrice = books.Max(b => b.Price);
        var deals = books.Where(b => b.Rating >= 4 && b.InStock)
            .Select(b => new BookDeal
            {
                Book = b,
                ValueScore = Math.Round(b.Rating * (1 - (b.Price / maxPrice)) * 100, 1),
                DealReason = b.Rating == 5
                    ? $"5-star book for only {b.PriceDisplay}!"
                    : $"Highly rated ({b.Rating}/5) at just {b.PriceDisplay}"
            })
            .OrderByDescending(d => d.ValueScore).Take(20).ToList();
    ...
    public async Task<IActionResult> ExportCsv()
    {
        ...
        return File(memoryStream.ToArray(), "text/csv", $"books-export-{DateTime.Now:yyyy-MM-dd}.csv");
    }
}